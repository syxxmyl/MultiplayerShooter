...
# Pitch in Multiplayer - 16. Pitch in Multiplayer
...
`Root Bone`设置为`upperarm_l`，指定`FABRIK算法`应当向上执行到哪块骨骼停止

用`AimOffsets cached pose` 和`LeftHandTransform`作为`FABRIK算法`的入参
